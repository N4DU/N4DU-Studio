...
import { FORMATS, detectEncodeSupport, renderAndEncode } from '../core/exporter.js';
...
export function syncControls() {
  ...
  $('modeHint').textContent = state.mode === 'original'
    ...
    : 'Arrastrá para mover el encuadre y usá la rueda o el slider para el zoom.';

  const hints = [];
  if (!FORMATS[state.fmt].alpha && state.roundness > 0)
    hints.push('El JPG no admite transparencia: fuera de la forma el fondo será blanco.');
  if (state.fmt === 'ico')
